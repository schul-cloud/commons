...
describe('test configuration error', () => {
	...
	it('create error without data', () => {
		...
		expect(error.message, 'contains stringified data').to.include('[no data defined]');
	});
	...
	it('create error with non-json data', () => {
		...
		expect(error.message, 'contains stringified data').to.include('hello world');
	});
	...
	it('create error with malformed data', () => {
		...
		expect(error.message, 'contains stringified data').to.include('[error retrieving data]');
	});
